...
class MahmoudCNVE(MahmoudTrj):
    ...
    def gipps_cf(self):
        ...
        '''
        vf = self.fol_veh.speed
        vfd = self.fol_veh.des_speed
        af = self.fol_veh.amax
        bf = self.fol_veh.amin

        bl = self.lead_veh.amin
        # vl will be looped over in the for loop
        Ll = self.lead_veh.length

        # d = self.fol_veh.dist - self.lead_veh.trj_d[indx] will be looped over in the for loop
        ...
        vf[0] = self.fol_veh.speed
        df[0] = self.fol_veh.dist
