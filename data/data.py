...
def get_general_params(inter_name):
    ...
    :return:
        ...
        - lli: A dictionary of sets. The key is the phase number and is one-based. The value to a key is a set of lanes included in that phase (lanes are also one-based)
        - pli: a dictionary of sets where the keys are lane numbers and must be one-based. The value for each key is a set of lane numbers that are in conflict with the key lane (again, must be one-based).
    ...
    if inter_name == "13th16th":
        ...
        return {"inter_name": "TERL",
                ...
                "small_positive_num": 0.01,
                "large_positive_num": 999_999_999,
                "lli": {1: {1, },  # Southbound (signal controller: phase 2)
